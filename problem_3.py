"""
Make the necessary modifications to the above 
computer program of the model of the checkout
counter (Example 4.2) so that an arriving 
customer does not join the queue if three
or more customers are waiting for service.

For this modification , 
Introducing a new variable for 
rejected customer if queue length
 is more than 3 (queue full).
...
class Sim:
    def __init__(self, total_customers, mean_interarrival_time, mean_service_time, sigma):
        ...
        self.LongService = 0
        self.RejectedCustomers = 0  # New variable to track rejected customers
    ...
    def process_arrival(self, event):
        # Check if there are less than 3 customers in the queue
        if self.QueueLength < 3:
            ...
            # Customer is rejected due to a full queue
            self.RejectedCustomers += 1

        # Schedule the next arrival if within the simulation time limit
        next_arrival_time = self.Clock + self.exponential(self.MeanInterArrivalTime)
        if next_arrival_time <= self.SimulationTimeLimit:
    ...
    def report_generation(self):
        ...
        print(f"\tNUMBER OF REJECTED CUSTOMERS: {self.RejectedCustomers}")  # New output
